...
describe("WeavePlugin integration", () => {
  ...
  it("config handler maps agent override modelOptions to options", async () => {
    const config = WeaveConfigSchema.parse({
      agents: {
        loom: {
          modelOptions: {
    ...
    const loomKey = getAgentDisplayName("loom")
    const loomAgent = result.agents[loomKey] as { options?: Record<string, unknown> }
    expect(loomAgent.options).toEqual({
      reasoningEffort: "high",
      reasoning: { effort: "medium" },
    })
  })
...
describe("createManagers — builtin display_name override", () => {
  ...
  it("custom agent modelOptions map to options end-to-end", async () => {
    const config = WeaveConfigSchema.parse({
      custom_agents: {
        reviewer: {
          prompt: "Review code.",
    ...
    const reviewer = result.agents["reviewer"] as { options?: Record<string, unknown> } | undefined
